...
export function useFileUpload() {
    ...
    const [filesWaitingForUpload, setFilesWaitingForUpload] = useAtom(filesWaitingForUploadAtom)
    ...
    const uploadFiles = useCallback(async (acceptedFiles: File[]) => {
        ...
        if (client && project && task && collection && userDocRef) {
            ...
            setFilesWaitingForUpload(acceptedFiles)
            setShowFolders(true)
        }
    }, [userDocRef, files, collection])

    useEffect(() => {
        if (filesWaitingForUpload.length > 0 && collection) {
            uploadFiles(filesWaitingForUpload)
            setFilesWaitingForUpload([])
        }
    }, [collection, filesWaitingForUpload])
